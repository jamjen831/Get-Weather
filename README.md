...
Get OpenWeatherMap info for BlueIris overlay or any other program that requires a plain-text file.  It should be pertty straight forward to modify the module to get other fields or polling options.  The module is now using the default JSON, allowing for polling additonal data.  Older HTML version preserved as Get-Weather-hmtl_old.psm1 file
